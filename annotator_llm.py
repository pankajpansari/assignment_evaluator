...
class AnnotatorLLM(base_llm.BaseLLM):
    ...
    def create_message_content(self):
        ...
        output_prefix = utils.CONFIG['assignment']['problem_file'].split('/')[1].split('.')[0]  
        aggregate_feedback = self.read_file(pathlib.Path(utils.CONFIG['assignment']['intermediate_path']) / f"{output_prefix}_aggregator_{utils.CONFIG['aggregator']['model']['type']}_{self.parameter}.txt")
